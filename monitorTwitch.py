...
import twitch_chat_irc as twitch_chat_irc 

RANDOM = np.random.RandomState(cfg.RANDOM_SEED)

USERNAME = "TwitchUserName"
TOKEN = "oauth:30CharacterKey"
CHANNEL = "TwitchChannel"

BIRDWEATHER_ID = "99999"
BIRDWEATHER_LAT = "00.00"
BIRDWEATHER_LON = "00.00"
...
def monitorStream( cfg ):
    ...
    #connection.send( CHANNEL, 'Starting BirdNet-Analyzer')
                            ...
                            if last_bird != p_bird[0] or time.time() > last_msg_time + 30:
                                try:
                                    if p_bird[0] not in lstCommon:
                                        connection.send( CHANNEL, twitchMessage)
                                    else:
                                        connection.send( CHANNEL, twitchMessage)
                                except:
                                    connection = twitch_chat_irc.TwitchChatIRC(USERNAME, TOKEN)
